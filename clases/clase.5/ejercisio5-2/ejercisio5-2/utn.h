...
void utn_ingresoVector(int *pVector,char *mensajeIngreso, char *mensajeError , int cantidad);//ingreso de numero entero en vector
void utn_getVecMaxMin(int *pVector,char *mensajeIngreso, char *mensajeError , int cantidad, int minimo, int maximo);//ingreso numeros enteros en un vector con maximo y minimo validado
int utn_sumaVector(int *pResultado, int *pVector, int cantidad);//suma los numeros dentro de un vector
void utn_printVector(int *Pvector,char *mensaje, int cantidad);//imprime los espacios que se desee de un vector, el mensaje simepre empezara diciendo "el %d ", i++
